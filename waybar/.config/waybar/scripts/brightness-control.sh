...
case "$1" in
    "get")
        # Fallback to 50 if current is empty to avoid Waybar crashing
        val=${current:-50}
        # Output JSON for Waybar
        # 'text' is what stays on the bar, 'tooltip' is what shows on hover
        echo "{\"text\": \" \", \"tooltip\": \"Brightness: $val%\", \"percentage\": $val}"
        ;;
    "up")
